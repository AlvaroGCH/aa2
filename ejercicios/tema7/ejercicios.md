...
|------------|-------------|
| **Percepción** (¿Qué información recibe?) | Mensaje del cliente vía chat, email o WhatsApp |
...
| **Decisión** (¿Cómo procesa?) | Un LLM (ej: GPT-4o o Claude) analiza el mensaje, identifica la intención (consulta de pedido, devolución, queja) y determina la acción apropiada |
...
| **Acción** (¿Qué ejecuta?) | Responder al cliente con información del pedido, iniciar proceso de devolución en el ERP, escalar ticket a agente humano, enviar email de confirmación |
...
3. **Trade-offs**: n8n tiene menor número de integraciones predefinidas (pero se puede suplir con HTTP Request), curva de aprendizaje mayor que Zapier (mitigable con templates y documentación), y necesidad de mantener la infraestructura propia (mitigable con Docker y backups automatizados).
...
| Hora | 8 | Inicio de la jornada laboral |
| Minuto | 0 | En punto |
| Días de la semana | Lunes a Viernes (1-5) | Solo días laborables |
| Zona horaria | Europe/Madrid | Hora local española |

**Pregunta**: Si el servidor está apagado a las 8:00, n8n NO ejecutará el workflow retroactivamente cuando vuelva a estar online (por defecto). Las ejecuciones perdidas se pierden. Para mitigar esto, es recomendable monitorizar la disponibilidad del servidor y/o configurar alertas.
...
**Pregunta**: Usar `$credentials` es fundamental porque: (1) la API key se almacena cifrada en n8n, (2) no se expone al exportar el workflow, (3) se puede revocar y actualizar sin modificar el workflow, y (4) se puede compartir entre múltiples workflows. Las credenciales se configuran en Settings > Credentials en n8n.
...
|----------|---------------------|
| "AI Agent" | Varía según fecha (ej: ~50+) |
| "OpenAI" | Varía según fecha (ej: ~200+) |
| "chatbot" | Varía según fecha (ej: ~100+) |
| "email automation" | Varía según fecha (ej: ~150+) |
...
| URL | https://n8n.io/workflows/ (varía según templates disponibles) |
...
| ¿Qué herramientas (tools) usa el agente? | Ninguna adicional (solo conversación) |
| ¿Qué trigger lo inicia? | Chat Trigger (interfaz de chat embebida) |
| Complejidad estimada (Baja/Media/Alta) | Baja |
| ¿Podrías usarlo como base para un proyecto propio? | Sí, ideal para empezar con agentes conversacionales |
...
| Nodos que utiliza (listado) | Chat Trigger, AI Agent, OpenAI Chat Model, SerpAPI Tool, Calculator Tool, HTTP Request Tool |
| ¿Usa nodo AI Agent? | Sí |
| ¿Incluye memoria? | Sí |
| ¿Qué herramientas (tools) usa el agente? | SerpAPI (búsqueda web), Calculator, HTTP Request |
| ¿Qué trigger lo inicia? | Chat Trigger |
| Complejidad estimada (Baja/Media/Alta) | Media |
| ¿Podrías usarlo como base para un proyecto propio? | Sí, excelente ejemplo de agente con múltiples herramientas |
...
| Nodos que utiliza (listado) | Webhook Trigger, AI Agent, OpenAI Chat Model, Postgres (memoria), Vector Store Tool |
| ¿Usa nodo AI Agent? | Sí |
| ¿Incluye memoria? | Sí (PostgreSQL para persistencia) |
| ¿Qué herramientas (tools) usa el agente? | Vector Store (búsqueda semántica en documentos), Webhook (para integraciones) |
| ¿Qué trigger lo inicia? | Webhook Trigger |
| Complejidad estimada (Baja/Media/Alta) | Alta |
| ¿Podrías usarlo como base para un proyecto propio? | Sí, perfecto para casos de soporte con base de conocimiento |
...
2. **Trigger más frecuente**: Chat Trigger y Webhook Trigger son los más utilizados. Chat Trigger es ideal para interacciones conversacionales directas, mientras que Webhook permite integraciones con servicios externos (Slack, Discord, apps custom).

3. **Memoria**: Los templates con memoria suelen usar Window Buffer Memory (para conversaciones recientes), PostgreSQL (para persistencia a largo plazo) o Vector Store (para búsquedas semánticas). El tipo de memoria afecta cuánto contexto recuerda el agente y cómo lo recupera.
